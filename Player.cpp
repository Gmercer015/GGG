...
void Player::FireG(bool* Game, int pN, Map& screen, char map[][20], char bgmap[][20], int xcord, int ycord) // FIRE function
{
    ...
    if (pN == 2)
    {
        while (map[ycord][xcord - Path] != 'G' && map[ycord][xcord] != '#')
        {
            ...
            screen.cls();
            screen.RefreshScreen(); *Game = screen.Game(); // set bool to check if a player is killed
        }
        delay(200); // delay 2 seconds
        for (int i = 19; i--; ) // run through the x coordinates of the graph, deleting all H's found
        {
            if (map[ycord][i] == 'H')
            {
                map[ycord][i] = bgmap[ycord][i];
            }
        }
    }
    else
